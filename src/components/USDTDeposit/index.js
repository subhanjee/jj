...
import WithdrawUSDT from '../withdrawal';

export default function USDTDeposit() {
  const [isQRModalOpen, setIsQRModalOpen] = useState(false);
  const [activeTab, setActiveTab] = useState('deposit'); // deposit | withdraw | reward
  ...
  const renderTabs = () => (
    <div className="flex space-x-4">
      {['Deposit USDT', 'Withdraw USDT', 'Reward'].map((tabLabel) => {
        const key = tabLabel.toLowerCase().split(' ')[0]; // deposit, withdraw, reward
        return (
          <button
            key={key}
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between bg-[#1e1e1e] border border-gray-600 rounded-lg px-4 py-3">
                  <span className="text-sm break-all text-orange-500">{depositAddress}</span>
